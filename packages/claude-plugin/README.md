...
This plugin lives in an Nx monorepo. Its version in `.claude-plugin/plugin.json` and the plugin
entry in the repo-root `.claude-plugin/marketplace.json` are kept in sync from this package's
`package.json` by `nx release` (see the root README).
